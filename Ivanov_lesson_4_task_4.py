...
def get_parc(resp, valute, text):
    ...
    :param valute: код валюты, для которого получаем данные
...
def currency_rates(arg):
    ...
    :return: Возвращает список из даты, наименования валюты и собственного курса
    """
    arg = arg.upper()
    ...
    if response.text.find(arg) != -1:
        ...
        my_date = datetime.datetime.strptime(response.text.split("Date")[1].split('"')[1].split('"')[1], "%d.%m.%Y").date()
